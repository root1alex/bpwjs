...
test('# quetions epic - getAllQuestion success', () => {
  const payload = {id: 0};
...
test('# questions epic - getAllQuestion error', () => {
  const input = {type: ActionTypes.GET_ALL_QUESTIONS, payload: {}};
  const input$ = ActionsObservable.from([input]);

  getAllQuestions(input$)
    .subscribe((res) => {
      expect(res.type).toBe(ActionTypes.GET_ALL_QUESTION_ERROR);
...
test('# questions epic - answerQuestion success', () => {
  ...
  Rx.Observable.ajax.get = post;

  answerQuestion(input$)
    .subscribe((res) => {
      expect(post.mock.calls.length).toBe(1);
      expect(post.mock.calls[0][0])
        .toBe('http://localhost:8080/api/question/0/answer');
...
test('# questions epic - createQuestion success', () => {
  ...
  createQuestion(input$)
    .subscribe((res) => {
      expect(post.mock.calls.length).toBe(1);
      expect(post.mock.calls[0][0])
        .toBe('http://localhost:8080/api/question/');
...
test('# questions epic - createQuestion error', () => {
  const input = {type: ActionTypes.CREATE_QUESTION, payload: {question: {id: 0}}};
  const input$ = ActionsObservable.from([input]);

  createQuestion(input$)
    .subscribe((res) => {
      expect(res.type).toBe(ActionTypes.CREATE_QUESTION);
      expect(res.payload.error.AjaxError.message).toBe('ajax error');
    });
});
...
test('# questions epic - deleteQuestion error', () => {
  ...
  deleteQuestion(input$)
    .subscribe((res) => {
      expect(res.type).toBe(ActionTypes.DELETE_QUESTION);
      expect(res.payload.error.AjaxError.message).toBe('ajax error');
    });
});
